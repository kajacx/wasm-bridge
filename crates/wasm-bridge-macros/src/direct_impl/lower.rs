...
pub fn lower_struct(name: Ident, data: DataStruct) -> TokenStream {
    ...
    let name_impl = format_ident!("impl_lower_{}", name.to_string().to_snake_case());
    quote!(
      mod #name_impl {
        ...
        impl wasm_bridge::direct::Lower for #name {
            fn to_js_args<M: wasm_bridge::direct::WriteableMemory>(&self, args: &mut wasm_bridge::direct::JsArgsWriter, memory: &M) -> wasm_bridge::Result<()> {
                #to_abi_impl
                Ok(())
            }
...
pub fn lower_variant(name: Ident, data: DataEnum) -> TokenStream {
    ...
    let to_js_args = quote!(
        ...
        // Subtract an additional 1 to account for the initial variant tag
